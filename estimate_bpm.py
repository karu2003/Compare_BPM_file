...
class BPM_Analyzer:
    ...
    def computeWindowBPM(self,data, framerate):
        ...
        min_idx = int(60. / 320 * (framerate/max_downsample))
        ...
        #     mean = np.mean(dC)
        #     # Normalize the data just by subtracting the mean
        #     dCs[i] = dC - mean
        ...
        # We're going to perform the autocorrelation on the summation of each band
        dC_sum = np.zeros([coeff_minlen])
        for dC in dCs:
            # To sum the bands up, each length needs to be exactly the same. Here's where coeff_minlen is handy!
            dC_sum += dC[:coeff_minlen]
        # # We're looking for pattern similarities at different times, which correspond to beats!

        # dC_sum = np.zeros(len(dCs[0]))
        # for dC in dCs:
        #     dC_sum += dC

        # dC_sum = np.zeros(len(dCs[0]))
        # for dC in dCs:
        #     dC_sum = sum(dC_sum,dC)
        ...
        x = correlation_half[min_idx:max_idx]
        x_max = np.max(x)
        peaks, properties  = signal.find_peaks(x, prominence=2, width=12, height=x_max*0.3)

        # peak_idx = self.detectPeak(correlation_half[min_idx:max_idx])[0] + min_idx
        # # Just like when we were determining the bounds, we can use the same formula to determine the bpm

        # if len(peak_idx) == 0:
        #     return print("no_audio_data")

        if len(peaks) > 1:
            peaks_diff = np.diff(peaks)
            diff_mean = np.mean(peaks_diff)
            bpm = round(60. / diff_mean * (framerate/max_downsample),2)
            if bpm > 220:
                bpm = int(bpm/2)
        else:
            peak = peaks[0]
            bpm = round(60. / (peak + min_idx) * (framerate/max_downsample),2)    
        return bpm
